...
try {
    ...
    if (!password_verify($password, $user['password_hash'])) {
        ...
        http_response_code(401);
        echo json_encode(['success' => false, 'message' => 'Invalid email or password']);
        logActivity($user['id'], 'login_failed', 'Wrong password');
        exit;
    }
    ...
    // Store session in database if remember_me
    if ($remember_me) {
        $session_id = session_id();
        $expires = date('Y-m-d H:i:s', strtotime('+30 days'));
        
        $stmt = $conn->prepare("INSERT INTO sessions (id, user_id, ip_address, user_agent, last_activity, expires_at) VALUES (?, ?, ?, ?, NOW(), ?)");
        $stmt->bind_param("sisss", $session_id, $user['id'], $ip_address, $_SERVER['HTTP_USER_AGENT'], $expires);
        $stmt->execute();
        $stmt->close();
    }
